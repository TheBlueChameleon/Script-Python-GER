...
Es ist auch möglich, mehrere Plot-Fenster gleichzeitig zu steuern. Außerdem kann Python Code \emph{parallel zum Fenster} ausgeführt werden: normalerweise wird die Code-Ausführung mit \texttt{plt.show()} angehalten, bis das Plot-Fenster geschlossen wird. Im folgenden zeige ich, wie man Python-Code dazu bringt, gleichzeitig zum Betreiben des/der Plot-Fenster(s) weiter den Code auszuführen.

Bisher haben wir alle Befehle der MatPlotLib direkt über den Alias \texttt{plt} aufgerufen. Tatsächlich handelt es sich dabei aber um Methoden von Klassen, die im Hintergrund der MatPlotLib agieren. Besondere Bedeutung haben dabei die Klassen \texttt{Figure} und \texttt{AxesSubplot}.
...
Hier erfolgt die Ausgabe von Zeile 19 schon während der Plot dargestellt wird, und nicht erst, nachdem das Plot-Fenster geschlossen wird. Aus demselben Grund ist auch Zeile 20 notwendig: Der Code läuft nach \texttt{fig.show()} weiter, bis die letzte Zeile ausgeführt wurde; danach beendet Python alle Prozesse, die mit dem Code zu tun hatten, einschließlich der Darstellung des Plots. Mit \inPy{input} zwingen wir Python dazu, auf eine Usereingabe zu warten.
